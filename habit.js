...
arrowBtn1.addEventListener("click", function() {
    ...
    if (habitContent1.style.display === "none") {
...
const meditationProgress =
    document.getElementById("meditationProgress");

 const meditationMessage =
    document.getElementById("meditationMessage");

    const savedMeditation =
    JSON.parse(localStorage.getItem("meditationProgress")) || [];

    function updateMeditationProgress() {

    const completedDaysCount =
        document.querySelectorAll("#habitContent1 .day-btn.completed").length;

    meditationProgress.textContent =
 ...
 dayButtons.forEach(function(button) {
    ...
    button.addEventListener("click", function() {
        ...
        const completedDays =Array.from(dayButtons).filter(function(btn) {
